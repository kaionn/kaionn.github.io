...
export default function Career() {
  return (
    <Section id="career" heading="Career" index={4}>
      <Panel>
        <div className="space-y-3">
          <div className="grid grid-cols-[minmax(0,1fr)_minmax(0,1fr)_minmax(0,2fr)] gap-x-4 border-b border-line pb-2 font-mono text-xs uppercase tracking-wide text-muted">
            <span>期間</span>
            <span>所属 / 役割</span>
            <span>担当領域</span>
          </div>
          {CAREER_ENTRIES.map((entry) => (
            ...
            >
              ...
              <span className="text-sm text-text/80">
                ...
                <span className="block text-muted">{entry.role}</span>
              </span>
              <span className="text-sm leading-relaxed text-text/80">
                {entry.focus}
              </span>
            </div>
          ))}
