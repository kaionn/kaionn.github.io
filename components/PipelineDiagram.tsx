...
export default function PipelineDiagram() {
  return (
    <div
      className="overflow-x-auto rounded-md border border-line bg-white/60 p-4"
      role="img"
      ...
      <ol className="flex w-max items-center gap-2 font-mono text-xs sm:text-sm">
        {PIPELINE_STEPS.map((step, index) => (
          <li key={step} className="flex items-center gap-2">
            <span className="whitespace-nowrap rounded border border-line bg-paper px-3 py-2 text-ink">
              {step}
            </span>
